...
public interface MainFormCommand extends Command {
    ...
    static void throwOnInvalidStateTransition(MainFormState state, Command c) {
        if (state.getOffChainVersion() == null) {
            if (isCommandCreate((MainFormCommand)c)) {
                return;
            }
    ...
    static boolean isCommandCreate(MainFormCommand c) {
        if (c.getOffChainVersion().equals(MainFormState.VERSION_NULL))
            return true;
        return false;
    }
